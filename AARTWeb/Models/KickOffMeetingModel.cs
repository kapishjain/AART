...
namespace AARTWeb.Models
{
    public class KickOffMeetingModel : BaseModel
    {
        ...
        public List<AssgnActProdList> UpdateAssgnActPro(AssgnActProdList pObj) {
            using (var httpClient = new HttpClient())
            {
                var url = ConfigurationManager.AppSettings["WEBAPIURL"];
        ...
        public List<AssgnSecProdList> UpdateAssgnSecPro(AssgnSecProdList sObj)
        {
            using (var httpClient = new HttpClient())
            {
                var url = ConfigurationManager.AppSettings["WEBAPIURL"];
        ...
        public string InsertProDocKOMDtls(List<KickOffMeetingModel.ProDocAttendies> models, List<ProDocActivityVo> docactivity, List<ProDocSectionAssignmentVO> docsection)
        {
            List<ProDocAttendiesVO> lstattendies = new List<ProDocAttendiesVO>();
            List<ProDocActivityVo> lstactivity = new List<ProDocActivityVo>();
            List<ProDocSectionAssignmentVO> lstdoc = new List<ProDocSectionAssignmentVO>();
            ...
            foreach (var i in models)
            {
                ...
                objattendies.Created_Date = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss");
                lstattendies.Add(objattendies);
            }
            foreach (ProDocActivityVo objactivity in docactivity)
            {
                objactivity.Created_By = HttpContext.Current.Session["UserID"].ToString();
                objactivity.Created_Date = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss");
                objactivity.Status = "A";
                lstactivity.Add(objactivity);
            }
            foreach (ProDocSectionAssignmentVO obsection in docsection)
            {
                obsection.Created_By = HttpContext.Current.Session["UserID"].ToString();
                obsection.Created_Date = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss");
                obsection.Status = "A";
